...
export const RenaissanceNavbar: React.FC = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 py-4 px-4 sm:px-8 lg:px-12 flex items-center justify-between text-xs font-sans select-none transition-all duration-500 ease-in-out ${
        isScrolled
          ? 'bg-[#ddddd1]/90 backdrop-blur-md border-b border-neutral-400/30 shadow-xs'
    ...
    >
      ...
      {/* Nav Links */}
      <nav
        className={`hidden md:flex items-center gap-8 text-xs font-bold transition-colors duration-300 ${
      ...
      {mobileNavOpen && (
        <div className="md:hidden absolute top-full left-4 right-4 mt-2 p-6 bg-[#e6e6dc]/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-neutral-300 text-sm space-y-4 text-black animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <a href="#control" onClick={() => setMobileNavOpen(false)} className="block font-bold">
            Control
          </a>
          <a href="#evidence" onClick={() => setMobileNavOpen(false)} className="block font-bold">
            Evidence
          </a>
          <a href="#engineering" onClick={() => setMobileNavOpen(false)} className="block font-bold">
            Engineering
          </a>
          <a href="#docs" onClick={() => setMobileNavOpen(false)} className="block font-bold">
            Docs
          </a>
          <div className="pt-2">
            <Type1Button
